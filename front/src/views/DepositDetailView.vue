  ...
  <div class="outer container">
    <div class="link">
      <p class="bread-crumb">
        ...
        <a href="http://localhost:5173/compare/deposit">정기예금</a> >
        <a :href="`http://localhost:5173/compare/deposit/${route.params.id}`">{{
          product.fin_prdt_nm
        }}</a>
      </p>
...
const get_optionList = () => {
  const optionListString = store.userInfo.deposit_option_list;

  if (optionListString !== null) {
    deposit_optionlist.value = optionListString.split(",").map(Number);
  } else {
...
const isOptionSelected = (optionId) => {
  return deposit_optionlist.value.includes(optionId);
};

onMounted(() => {
  product.value = store.deposit[route.params.id - 1];
  if (store.isLogin){
    get_optionList();
  }
});
...
const toggleOption = (optionId) => {
  const index = deposit_optionlist.value.indexOf(optionId);
  if (index === -1) {
    // 배열에 없는 경우 추가
    deposit_optionlist.value.push(optionId);
  } else {
    // 배열에 있는 경우 제거
    deposit_optionlist.value.splice(index, 1);
  }
};
...
const submitRegistration = () => {
  ...
  if (deposit_optionlist.value.length > 0) {
    selectedOptionStr = deposit_optionlist.value.join(",");
  }
  editInfo.value = store.userInfo;
  editInfo.value.deposit_option_list = selectedOptionStr;
